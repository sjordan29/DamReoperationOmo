...
def calcFlows(filename,dates):
    ...
    simulatedFlow_18 = np.zeros(numFlowDays)
    simulatedFlow_TR = np.zeros(numFlowDays)

    numSubB = 27
    flowSubB_18 = 17 # after Koysha 
    ...
    flows = pd.DataFrame({'date':dates, 'koy':simulatedFlow_18, "sug":simulatedFlow_RA, 'cot':simulatedFlow_EF, 'tur':simulatedFlow_TR})
