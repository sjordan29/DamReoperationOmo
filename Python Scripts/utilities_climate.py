...
def combineData(fpath, mc_dict, lc_dict, npol, Objectives):
    '''
...
def PlotIt(x_ls, table, color, linestyle, label, ax,d,s,u):
    ...
    xs = x_ls
    for col in table.columns:
        ax.plot(xs, table[col], c=color,
                linewidth = 3 if label == "Robust Policy" else lw,
                label=label if (d==1) or (s==1) or (u==1) else "",
                linestyle=linestyle,
                zorder = 2 if label == "Robust Policy" or label == "Uncontrolled" else 1,
                alpha=1 if (label=='Robust Policy' or label == "Uncontrolled") else av, 
                path_effects=[pe.Stroke(linewidth=5, foreground='k'), pe.Normal()] if label == 'Robust Policy' else None)
        d+=1 
        s+=1
        u+=1
...
def PAP_ltd(ax, dps_dict, uc_dict, obj, dps_cols, dps_addl, uc_addl, x_ls, cmap, col_dict, uc_huc = None, huc_blw=None):
    dps_table = dps_dict[obj]
    uc_table = uc_dict[obj]
    ...
    mn = min([dps_mn, uc_mn, dps_mn_addl, uc_mn_addl])
    mx = max([dps_mx, uc_mx, dps_mx_addl, uc_mx_addl])
    ...
    s=1
    d=1
    for item in dps_cols:
        PlotIt(x_ls, dps_scaled[[item]], cmap(col_dict[int(item[1:])]), 'solid', 'Robust Policy', ax, d, s, u)
    u=1
